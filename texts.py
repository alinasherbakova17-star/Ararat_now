TEXTS = {
    "ru": {
        "welcome": "🏔 <b>Ararat Now</b>\n\n"
           "Бот, который говорит, видно ли сегодня Арарат\n\n"
           "🔔 При подписке:\n"
           "— утром ты получишь статус видимости\n"
           "— вечером — подборку лучших фото дня 📸\n\n"
           "Выбери язык:",

        "check_prompt": "Теперь можешь проверить видимость, включить уведомления или отправить фото. "
                "Нажми /check_now, чтобы проверить видимость\n\n"
                "<b>Нажми 🔔, чтобы не пропускать араратные дни</b>\n\n",


        "subscribe_button": "🔔 Включить уведомления",
        "auto_subscribed_after_check": "🔔 Я включил уведомления: теперь буду присылать статус, когда есть смысл смотреть на Арарат. Отключить можно кнопкой 🔕.",
        "unsubscribe_button": "🔕 Отключить уведомления",
        "photo_button": "📸 Отправить фото",
        "check_now_button": " 👀 Проверить сейчас",
        "oracle_button": "🔮 Араратный оракул",

        "subscribed_text": "Уведомления включены",
        "unsubscribed_text": "Уведомления отключены",

        "no_language": "Сначала выбери язык",

        "photo_prompt": "Отправь фото Арарата 📸",
        "photo_received": "📸 Фото принято. Момент зафиксирован.",
        "photo_caption_prefix": "Новое фото Арарата",
        "best_photo_day_caption": "📸 Лучшие фото дня\n\nСегодня Арарат поймали так 👀",

        "sky_label": "Небо",
        "temp_label": "Температура",
        "wind_label": "Ветер",
        "clouds_label": "Облачность",
        "visibility_label": "Видимость",
        "air_label": "Воздух",
        "decision_label": "Вердикт",

        # 🌤 статусы
        "excellent": [
            "Сегодня Араратный день",
            "Арарат сегодня не только на упаковке шоколада",
            "Вот это тот самый вид",
        ],
        "good": [
            "Вид есть, можно ловить момент",
            "Арарат сегодня в кадре",
            "Араратная погода",
        ],
        "cloudy": [
            "Облака тоже хотят внимания",
            "Облака плотные, Арарат прячется, но выглядывает",
            "Арарат частично виден, но облака мешают",
        ],
        "covered": [
            "Сегодня Арарат взял выходной",
            "Гора за плотной стеной облаков",
            "Слишком облачный день для идеального вида",
        ],
        "medium": [
            "Сегодня Арарат Шредингера: вроде бы есть, вроде бы нет",
            "Может появиться, может исчезнуть",
            "Сегодня всё нестабильно",
        ],
        "bad": [
            "Сегодня город без Арарата",
            "Гора полностью исчезла",
            "Сегодня не Араратный день",
        ],
        "smog": [
            "Арарат надел респираторную маску",
            "Смог скрывает Арарат",
            "Сегодня проблема не в облаках",
        ],

        # 🎯 решение
        "decision_text": {
            "excellent": "Вид идеальный, это Араратный день",
            "good": "Сегодня можно наслаждаться видом",
            "cloudy": "Сегодня облака хотят быть в центре внимания, Арарат частично выглядывает",
            "covered": "Гора закрыта облаками, сегодня лучше не ждать чёткого вида",
            "medium": "Всё нестабильно",
            "bad": "Арарат спрятался",
            "smog": "Арарат ушел искать чистый воздух",
        },

       "night_decision_text": {
          "excellent": "Даже ночью гора может читаться силуэтом, если знаешь куда смотреть",
          "good": "Ночью атмосфера красивее деталей, но контур может пойматься",
          "smog": "Ночь и дымка вместе спрятали Арарат",
          "bad": "Сейчас Арарат скорее в настроении, чем в видимости",
        },

        
        # 🌫 воздух
        "air_status": {
            "air_clean": "воздух чистый",
            "air_ok": "воздух нормальный",
            "air_heavy": "воздух тяжёлый",
            "air_bad": "воздух плохой",
        },

        # ☀️ tail (ТОЛЬКО good/excellent)
        "time_tail": {
            "morning": [
                "Утро начнается с проверки Арарата",
                "Бари луйс",
            ],
            "day": [
                "Арарат передает пламенный Барев",
                "Хороший день, чтобы прислать в бот фото Арарата",
            ],
            "evening": [
                "Арарат на закате - отдельный вид искусства",
                "Вечер обещает быть красивым",
            ],
            "night": [
                "Ночью больше про атмосферу, чем про детали. Арарат вернётся завтра.",
            ],
        },

        # 🔮 ORACLE
        "oracle": {
            "excellent": [
                "Арарат видно — значит, этот день уже удался.",
                "Проверь, что происходит в HelloBarevПривет, и выходи из дома скорее.",
                "Выбирайся на ужины и бранчи от Uptown Yerevan, чтобы найти полезные знакомства",
            ],
            "good": [
                "День, чтобы твоя самооценка была еще выше, чем Арарат",
                "Сегодня даже ереванские пробки не смогут тебя остановить.",
                "Если услышишь на Северном любимую песню — загадай желание.",
            ],
            "cloudy": [
                "Что-то важное уже рядом,\nно ты пока видишь только часть картины.",
                "Если тебе говорят «уже выхожу» — вспоминай армянские тайминги.",
                "Не бойся лишней чашки кофе.\nЭнергия пригодится.",
            ],
            "covered": [
                "Сегодняшний день нужно подсластить гатой.",
                "Это не «нет».\nЭто «ещё не время».",
                "Арарат даёт загадку, а от близких людей стоит ждать ясности.",
            ],
            "medium": [
                "Разреши эмоциональные качели только Арарату, а не другим людям",
                "Не всё должно быть чётко сегодня.",
                "Только Арарат может кататься между статусами так же, как ты между решениями",
            ],
            "bad": [
                "Арарат не видно.\nНо это не значит, что не получится.",
                "Пауза сильнее действия.",
                "Не ищи ясность там, где её нет.",
            ],
            "smog": [
                "Самое время купить домашние растения или очистители воздуха.",
                "Не доверяй первому впечатлению.",
                "Очисти голову, даже если воздух грязный.",
            ],
        },
    },

    "en": {
        "welcome":
            "🏔 <b>Ararat Now</b>\n\n"
            "A bot that tells you if Ararat is visible today\n\n"
            "🔔 With subscription:\n"
            "— morning visibility update\n"
            "— best photo of the day in the evening 📸\n\n"
            "Choose language:",
        "language_set": "Language set",
        "check_prompt": "Now you can check visibility, enable notifications, or send a photo\n\n"
                "<b>Push 🔔 to keep Ararat in your sights</b>\n\n",

        "subscribe_button": "🔔 Enable notifications",
        "unsubscribe_button": "🔕 Disable notifications",
        "photo_button": "📸 Send photo",
        "check_now_button": "👀 Check now",
        "oracle_button": "🔮 Ararat Oracle",

        "subscribed_text": "Notifications enabled",
        "auto_subscribed_after_check": "🔔 Notifications are now on: I’ll send updates when it makes sense to look for Ararat. You can turn them off with 🔕.",
        "unsubscribed_text": "Notifications disabled",

        "no_language": "Select language first",

        "photo_prompt": "Send an Ararat photo 📸",
        "photo_received": "📸 Photo received.",
        "photo_caption_prefix": "New Ararat photo",
        "best_photo_day_caption": "📸 Best photos of the day\n\nThis is how Ararat looked today 👀",

        "sky_label": "Sky",
        "temp_label": "Temperature",
        "wind_label": "Wind",
        "clouds_label": "Clouds",
        "visibility_label": "Visibility",
        "air_label": "Air",
        "decision_label": "Decision",

        "excellent": [
            "Everything is open today",
            "A rare day when Ararat does not hide",
            "This is the view",
        ],
        "good": [
            "You can catch the view",
            "Ararat is in the frame today",
            "A good day to look far",
        ],
        "cloudy": [
            "Clouds want attention too",
            "Heavy clouds — Ararat is hiding but peeking through",
            "Ararat is partly visible, but clouds are getting in the way"
        ],
        "covered": [
            "Ararat is completely hidden today",
            "The mountain is behind a wall of clouds",
            "Not a day to break through the sky",
        ],
        "medium": [
            "It could appear, it could disappear",
            "Everything is unstable today",
            "Conditions are on the edge",
        ],
        "bad": [
            "A city without Ararat today",
            "The mountain is gone from sight",
            "Don’t expect the view",
        ],
        "smog": [
            "Air ruins everything",
            "Smog takes away the horizon",
            "Today the problem is not the clouds",
        ],

        "decision_text": {
            "excellent": "Crystal clear view — it’s an Ararat day",
            "good": "A good day for the view",
            "cloudy": "Clouds are stealing the spotlight today, Ararat is only partly visible",
            "covered": "The mountain is hidden behind clouds, don’t expect a clear view today",
            "medium": "Conditions are unstable",
            "bad": "Ararat is hiding",
            "smog": "Ararat went to find cleaner air"
        },

        "night_decision_text": {
            "excellent": "Even at night, the mountain may appear as a silhouette if you know where to look",
            "good": "At night it is more about atmosphere than detail, but the outline may still appear",
            "smog": "Night and haze have hidden Ararat from view",
            "bad": "Right now Ararat is more of a mood than a visible scene",
        },

        "air_status": {
            "air_clean": "clean air",
            "air_ok": "normal air",
            "air_heavy": "heavy air",
            "air_bad": "bad air",
        },

        "time_tail": {
            "morning": [
                "A clear start",
                "This is how a good day begins",
            ],
            "day": [
                "The day is on your side",
                "Today might go better than usual",
            ],
            "evening": [
                "Sunset won’t disappoint",
                "The evening may turn beautiful",
            ],
            "night": [
                "Even at night, it’s worth looking up",
            ],
        },

        "oracle": {
            "excellent": [
                "If Ararat is visible, the day is already a success.",
                "Check what’s happening at HelloBarevPrivet and get out of the house.",
                "A day without fog.\nLook straight and act straight.",
            ],
            "good": [
                "Go for brunch with Uptown Yerevan and make useful connections.",
                "Today even Yerevan traffic won’t stop you.",
                "If you hear your favorite song on Northern Avenue, make a wish.",
            ],
            "cloudy": [
                "Something important is close,\nbut you can only see part of the picture.",
                "If someone says “I’m on my way”, remember Armenian timing.",
                "Don’t be afraid of one more coffee.\nYou may need the energy.",
            ],
            "covered": [
                "This is not a day for full clarity.",
                "It does not mean no.\nIt means not yet.",
                "Leave some mystery in the air.",
            ],
            "medium": [
                "A day in between.\nBetter to observe.",
                "Not everything needs to be clear today.",
                "Let the answer stay a hint.",
            ],
            "bad": [
                "Ararat is not visible.\nBut that does not mean nothing will work out.",
                "Pause is stronger than action.",
                "Don’t look for clarity where there is none.",
            ],
            "smog": [
                "There is too much noise around.",
                "Don’t trust the first impression.",
                "Clear your head even if the air is dirty.",
            ],
        },
    },

    "hy": {
        "welcome":
            "🏔 <b>Ararat Now</b>\n\n"
            "Բոտ, որը ցույց է տալիս՝ այսօր երևո՞ւմ է Արարատը\n\n"
            "🔔 Բաժանորդագրվելու դեպքում՝\n"
            "— առավոտյան կստանաս տեսանելիության ստատուսը\n"
            "— երեկոյան՝ օրվա լավագույն լուսանկարը 📸\n\n"
            "Ընտրիր լեզուն:",
        "language_set": "Լեզուն ընտրված է",
        "check_prompt": "Հիմա կարող ես ստուգել տեսանելիությունը, միացնել ծանուցումները կամ ուղարկել լուսանկար\n\n"
                "<b>Սեղմիր 🔔, որպեսզի Արարատյան օրերը բաց չթողնես</b>\n\n",

        "subscribe_button": "🔔 Միացնել ծանուցումները",
        "auto_subscribed_after_check": "🔔 Ծանուցումները միացված են․ կուղարկեմ ստատուս, երբ արժե նայել Արարատին։ Կարող ես անջատել 🔕 կոճակով։",
        "unsubscribe_button": "🔕 Անջատել ծանուցումները",
        "photo_button": "📸 Ուղարկել լուսանկար",
        "check_now_button": "👀 Ստուգել հիմա",
        "oracle_button": "🔮 Արարատն օրաքուլ",

        "subscribed_text": "Ծանուցումները միացված են",
        "unsubscribed_text": "Ծանուցումները անջատված են",

        "no_language": "Սկզբում ընտրիր լեզուն",

        "photo_prompt": "Ուղարկիր Արարատի լուսանկար 📸",
        "photo_received": "📸 Լուսանկարը ընդունված է։",
        "photo_caption_prefix": "Արարատի նոր լուսանկար",
        "best_photo_day_caption": "📸 Օրվա լավագույն լուսանկարները\n\nԱյսպես էր այսօր Արարատը 👀",

        "sky_label": "Երկինք",
        "temp_label": "Ջերմաստիճան",
        "wind_label": "Քամի",
        "clouds_label": "Ամպեր",
        "visibility_label": "Տեսանելիություն",
        "air_label": "Օդ",
        "decision_label": "Եզրակացություն",

        "excellent": [
            "Այսօր ամեն ինչ բաց է",
            "Հազվադեպ օր, երբ Արարատը չի թաքնվում",
            "Ահա հենց այս տեսարանն է",
        ],
        "good": [
            "Տեսարանը կա, կարելի է որսալ պահը",
            "Այսօր Արարատը կադրում է",
            "Լավ օր է հեռուն նայելու համար",
        ],
        "cloudy": [
            "Վերևից ծանր է, բայց հեռվում դեռ բաց կա",
            "Ամպերը խիտ են, բայց Արարատը դեռ երևում է",
            "Ամպերը խանգարում են, բայց չեն հաղթում",
        ],
        "covered": [
            "Այսօր Արարատը ամբողջովին թաքնված է",
            "Լեռը ամպերի պատի հետևում է",
            "Այսօր երկինքը չի բացվելու",
        ],
        "medium": [
            "Կարող է երևալ, կարող է անհետանալ",
            "Այսօր ամեն ինչ անկայուն է",
            "Պայմանները սահմանագծին են",
        ],
        "bad": [
            "Այսօր քաղաքը առանց Արարատի է",
            "Լեռը անհետացել է տեսադաշտից",
            "Մի սպասիր տեսարանին",
        ],
        "smog": [
            "Օդը փչացնում է ամեն ինչ",
            "Սմոգը խլում է հորիզոնը",
            "Այսօր խնդիրը ամպերը չեն",
        ],

        "decision_text": {
            "excellent": "տեսարանը իդեալական է",
            "good": "տեսանելի է",
            "cloudy": "տեսանելի է, բայց ամպերը խանգարում են",
            "covered": "լեռը ամբողջովին փակված է",
            "medium": "անկայուն պայմաններ",
            "bad": "չի երևում",
            "smog": "օդը խեղդում է տեսարանը",
        },

        "night_decision_text": {
            "excellent": "Նույնիսկ գիշերը լեռը կարող է ուրվագծով երևալ, եթե գիտես ուր նայել",
            "good": "Գիշերը սա ավելի շատ մթնոլորտի մասին է, քան մանրամասների, բայց ուրվագիծը կարող է երևալ",
            "smog": "Գիշերն ու մշուշը միասին թաքցրել են Արարատը",
            "bad": "Հիմա Արարատն ավելի շատ տրամադրություն է, քան տեսանելի պատկեր",
       },

        "air_status": {
            "air_clean": "մաքուր օդ",
            "air_ok": "նորմալ օդ",
            "air_heavy": "ծանր օդ",
            "air_bad": "վատ օդ",
        },

        "time_tail": {
            "morning": [
                "Մաքուր սկիզբ",
                "Այսպես է սկսվում լավ օրը",
            ],
            "day": [
                "Օրը քո կողմն է",
                "Այսօր կարող է ավելի լավ գնալ, քան սովորաբար",
            ],
            "evening": [
                "Մայրամուտը չի հիասթափեցնի",
                "Երեկոն կարող է շատ գեղեցիկ լինել",
            ],
            "night": [
                "Նույնիսկ գիշերը արժե վերև նայել",
            ],
        },

        "oracle": {
            "excellent": [
                "Եթե Արարատը տեսանելի է, օրը արդեն հաջողված է։",
                "Նայիր՝ ինչ է կատարվում HelloBarevPrivet-ում և շուտ դուրս եկ տնից։",
                "Օր առանց մառախուղի։\nՆայիր ուղիղ և գործիր ուղիղ։",
            ],
            "good": [
                "Գնա Uptown Yerevan-ի brunch-ին և օգտակար ծանոթություններ արա։",
                "Այսօր նույնիսկ երևանյան խցանումները քեզ չեն կանգնեցնի։",
                "Եթե լսես սիրելի երգդ Հյուսիսային պողոտայում, ցանկություն պահիր։",
            ],
            "cloudy": [
                "Ինչ-որ կարևոր բան մոտ է,\nբայց դու դեռ միայն մի մասն ես տեսնում։",
                "Եթե մեկը ասում է «արդեն գալիս եմ», հիշիր հայկական timing-ը։",
                "Մի վախեցիր ևս մեկ սուրճից։\nԳուցե ուժը պետք գա։",
            ],
            "covered": [
                "Այսօր ամբողջական հստակության օր չէ։",
                "Դա «ոչ» չի նշանակում։\nԴա նշանակում է «դեռ ոչ»։",
                "Մի քիչ առեղծված թող օդում։",
            ],
            "medium": [
                "Միջանկյալ օր է։\nԱվելի լավ է դիտել։",
                "Այսօր ամեն ինչ պարտադիր չէ, որ պարզ լինի։",
                "Թող պատասխանը մնա որպես ակնարկ։",
            ],
            "bad": [
                "Արարատը չի երևում։\nԲայց դա չի նշանակում, որ ոչինչ չի ստացվի։",
                "Դադարը ավելի ուժեղ է, քան գործողությունը։",
                "Մի փնտրիր պարզություն այնտեղ, որտեղ այն չկա։",
            ],
            "smog": [
                "Շուրջը չափազանց շատ աղմուկ կա։",
                "Մի վստահիր առաջին տպավորությանը։",
                "Մաքրի՛ր գլուխդ, նույնիսկ եթե օդը կեղտոտ է։",
            ],
        },
    },
}